...
The `*` and `*=` operators can be used to calculate the reverse Grassman product
of two quat's. The reason it's reversed is for uniformity with the matrix
generated by `math::rotation_mat(const quat& q)`.
