...
namespace
{
    ...
    TEST_CASE(vec2_times_mat2x2)
    {
        ...
        test_assert(v[0] == math::dot(fv2, dm22[0]));
        test_assert(v[1] == math::dot(fv2, dm22[1]));
    }

    TEST_CASE(vec2_times_mat3x2)
    {
        CONST_OR_CONSTEXPR auto v = fv2 * dm32;
        test_assert(v[0] == math::dot(fv2, dm32[0]));
        test_assert(v[1] == math::dot(fv2, dm32[1]));
        test_assert(v[2] == math::dot(fv2, dm32[2]));
    }

    TEST_CASE(vec2_times_mat4x2)
    {
        CONST_OR_CONSTEXPR auto v = fv2 * dm42;
        test_assert(v[0] == math::dot(fv2, dm42[0]));
        test_assert(v[1] == math::dot(fv2, dm42[1]));
        test_assert(v[2] == math::dot(fv2, dm42[2]));
        test_assert(v[3] == math::dot(fv2, dm42[3]));
    }

    TEST_CASE(vec3_times_mat2x3)
    {
        CONST_OR_CONSTEXPR auto v = fv3 * dm23;
        test_assert(v[0] == math::dot(fv3, dm23[0]));
        test_assert(v[1] == math::dot(fv3, dm23[1]));
    }

    TEST_CASE(vec3_times_mat3x3)
    {
        CONST_OR_CONSTEXPR auto v = fv3 * dm33;
        test_assert(v[0] == math::dot(fv3, dm33[0]));
        test_assert(v[1] == math::dot(fv3, dm33[1]));
        test_assert(v[2] == math::dot(fv3, dm33[2]));
    }

    TEST_CASE(vec3_times_mat4x3)
    {
        CONST_OR_CONSTEXPR auto v = fv3 * dm43;
        test_assert(v[0] == math::dot(fv3, dm43[0]));
        test_assert(v[1] == math::dot(fv3, dm43[1]));
        test_assert(v[2] == math::dot(fv3, dm43[2]));
        test_assert(v[3] == math::dot(fv3, dm43[3]));
    }

    TEST_CASE(vec4_times_mat2x4)
    {
        CONST_OR_CONSTEXPR auto v = fv4 * dm24;
        test_assert(v[0] == math::dot(fv4, dm24[0]));
        test_assert(v[1] == math::dot(fv4, dm24[1]));
    }

    TEST_CASE(vec4_times_mat3x4)
    {
        CONST_OR_CONSTEXPR auto v = fv4 * dm34;
        test_assert(v[0] == math::dot(fv4, dm34[0]));
        test_assert(v[1] == math::dot(fv4, dm34[1]));
        test_assert(v[2] == math::dot(fv4, dm34[2]));
    }

    TEST_CASE(vec4_times_mat4x4)
    {
        CONST_OR_CONSTEXPR auto v = fv4 * dm44;
        test_assert(v[0] == math::dot(fv4, dm44[0]));
        test_assert(v[1] == math::dot(fv4, dm44[1]));
        test_assert(v[2] == math::dot(fv4, dm44[2]));
        test_assert(v[3] == math::dot(fv4, dm44[3]));
    }
