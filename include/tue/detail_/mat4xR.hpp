...
public:
  ...
  explicit constexpr mat(const mat<T, 4, OtherR>& other) noexcept
    : impl_({{
        ...
        detail_::vec_utils<T, R>::resize(other[1], T(0), T(0)),
        detail_::vec_utils<T, R>::resize(other[2], T(1), T(0)),
        detail_::vec_utils<T, R>::resize(other[3], T(0), T(1)),
      }}) {
  }
  ...
  template<typename U>
  mat& operator*=(const mat<U, 4, 4>& other) noexcept {
...
template<typename T, int R>
inline constexpr auto operator+(const mat<T, 4, R>& m) noexcept {
...
namespace math
{
  ...
  inline auto pow(
    ...
    return mat<decltype(math::pow(base, exponent[0][0])), 4, R>{
      math::pow(base, exponent[0]),
      math::pow(base, exponent[1]),
      math::pow(base, exponent[2]),
      math::pow(base, exponent[3]),
    };
  }
