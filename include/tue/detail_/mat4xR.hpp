...
namespace tue
{
    ...
    namespace detail_
    {
        ...
        template<typename T>
        inline constexpr T det_m(const mat<T, 4, 4>& m) noexcept
        {
            return
                ( m[0][0] *
        ...
        template<typename T, int R>
        inline void swap_mm(mat<T, 4, R>& m1, mat<T, 4, R>& m2) noexcept
        {
            tue::detail_::swap_vv(m1[0], m2[0]);
            tue::detail_::swap_vv(m1[1], m2[1]);
            tue::detail_::swap_vv(m1[2], m2[2]);
            tue::detail_::swap_vv(m1[3], m2[3]);
        }

        template<typename T, int R>
        inline std::size_t hash_m(const mat<T, 4, R>& m) noexcept
        {
            return hash_v(m[0]) ^ hash_v(m[1]) ^ hash_v(m[2]) ^ hash_v(m[3]);
        }
