...
template<typename T>
inline constexpr auto operator+(const vec3<T>& v) noexcept {
...
namespace math
{
  ...
  inline auto pow(
    ...
    return vec3<decltype(math::pow(base, exponent[0]))>{
      math::pow(base, exponent[0]),
      math::pow(base, exponent[1]),
      math::pow(base, exponent[2]),
    };
  }
  ...
  template<typename T, typename U>
  inline constexpr auto select(
      const vec3<T>& condition,
      const vec3<U>& value,
      const vec3<U>& otherwise = vec3<U>::zero()) noexcept {
    return vec3<decltype(math::select(condition[0], value[0], otherwise[0]))>{
  ...
  template<typename T>
  inline vec3<T> rotation_vec(const vec3<T>& axis, const T& angle) noexcept {
    return axis * angle;
  }

  template<typename T>
  inline vec3<T> rotation_vec(
      const T& axis_x,
      const T& axis_y,
      const T& axis_z,
  ...
  template<typename T>
  inline vec3<T> rotation_vec(const vec4<T>& v) noexcept {
